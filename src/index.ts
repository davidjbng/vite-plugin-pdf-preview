    ...
    configureServer({ watcher, ws, config: { logger } }) {
      ...
      async function updatePdfPreview(file: string) {
        await generatePdf(previewFileUrl(file), {
          pdfPath: resolvePdfOutputPath(file),
        })
        ws.send({
          type: "full-reload",
          path: "*",
      ...
      const interval = setInterval(() => {
        if (config?.server.port) {
          ...
          for (const file of sourceFiles) {
            const pdfPath = resolvePdfOutputPath(file)
            generatePdf(previewFileUrl(file), { pdfPath })
            logger.info(
              colors.dim("File: ") +
                colors.blue(new URL(pdfPath, import.meta.url).href)
            )
          }
        }
      }, 50)
